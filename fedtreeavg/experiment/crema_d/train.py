...
from fed_multimodal.trainers.fed_tree_trainer import ClientFedTree



# define logging console: 로깅을 설정해 콘솔에 메시지를 포맷하고 기록
import logging
logging.basicConfig(
...
# 유틸리티 함수 1: 재현성을 위해 랜덤 시드를 설정합니다.
def set_seed(seed):
...
# 유틸리티 함수 2: 명령줄 인수를 파싱하고 설정 파일에서 경로 구성을 읽어옵니다.
def parse_args():
...
# 메인 훈련 루프: 데이터 로드, 모델 설정, 여러 에포크와 폴드 동안 훈련 루프를 실행합니다.
